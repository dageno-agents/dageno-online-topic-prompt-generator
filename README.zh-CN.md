...
5. 把每条实质业务线映射到最窄、稳定的 Canonical L3 市场边界。
6. 建立买家选择、比较、拒绝和验证供应商时的“竞争决策面”。
7. 把全部高价值意图聚类成带市场锚点的 Topic。
8. 为每个 Topic 生成不重复、可独立理解的 Prompt。
9. 用确定性 QA 检查覆盖、市场边界、重复、品牌词和证据。
...
L3 回答“这条业务线真正在哪个稳定市场竞争”。它位于品类需求、竞品研究和 Topic 生成之前。

L3 不等于 Topic：同一个 L3 可以拆出多个购买决策 Topic；多业务公司也可以映射多个 L3。正式行业基准只使用同 L3 需求，相邻市场进入竞争空白或范围外参考。

Skill 只读使用 Canonical 分类：只能复用调用方提供的当前 ID，不能自己发明生产 ID。没有目录匹配时输出待人工复核的候选市场。
...
系统不会因为“推荐、对比、价格、风险”各出现一次，就判断行业意图已经覆盖完整：

| 层级 | 示例 | 作用 |
| --- | --- | --- |
| 主意图 `it` | `comparison` 对比评估 | 稳定的报表分类 |
| 细分意图 `subIntent` | 指名品牌对比、采购标准比较、概念路线比较 | 用户真正要解决的决策问题 |
| 意图单元 `intentUnitId` | `supplier-quality-criteria-comparison` | 一个不可继续合并的语义问题 |

同一问题的措辞变体共用一个 `intentUnitId`，不会被当成多个行业需求重复计数。自动生成只有在重要意图单元已覆盖或被明确排除时才会停止。

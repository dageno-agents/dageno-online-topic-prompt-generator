...
import { requireReferences, validateBusiness, validateUnits, validateClusters, validatePromptBatch, evidenceAssessment, validateArtifact, panelDiff, validateVisibilityAssessment } from "./qa.mjs";
import { MONITORING_POLICY_VERSION, VISIBILITY_REVIEW_RULES, routeAssessment, metricUseFor, visibilitySummary } from "./visibility-policy.mjs";
...
export async function generatePanel(input, deps) {
  ...
  for (let pass = 0; pass < 2; pass++) {
    const gaps = await ask("coverage_challenger", "Act as an independent category researcher, NOT the generator. Audit the current inventory against original evidence and competitor capabilities. Find omitted buying stages, product lines, audiences, failure risks, alternatives and content demand; identify duplicate intent units. Specifically find missing genuine named-provider/product decisions behind commercially material knowledge-heavy scenarios; add these only with distinct real demand and sources, and use relatedContentUnitKeys to link supporting content units. Do not rewrite every how-to as best/top. Missing units must be material and evidenced, not mechanical permutations. Return empty arrays when no evidenced gaps remain. Do not declare internet-wide completeness. New surfaces need globally unique keys. Duplicates must be truly interchangeable decisions, not merely similar wording.", { business, config, intentOntology: INTENTS, surfaces, units, competitors: competitorResult.competitors, sources: evidencePayload(sources) }, gapSchema, data => {
    ...
    const mergedInto = new Map(gaps.duplicateUnits.map(d => [d.removeKey, d.keepKey]));
    for (const u of [...units, ...gaps.missingUnits]) if (u.relatedContentUnitKeys) u.relatedContentUnitKeys = [...new Set(u.relatedContentUnitKeys.map(k => mergedInto.get(k) || k))].filter(k => k !== u.key);
  ...
  for (const topic of selectedTopics) {
    const topicUnits = units.filter(u => topic.unitKeys.includes(u.key)).sort((a, b) => Number(b.pool === "monitoring_core" && b.scope === "brand_core") - Number(a.pool === "monitoring_core" && a.scope === "brand_core") || rank(a.priority) - rank(b.priority));
  ...
  for (const batch of chunks(selectedUnits, 12)) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const review = await ask("semantic_review", VISIBILITY_REVIEW_RULES + "\n\n" + "Independently review EVERY supplied question in its original language against its intent unit and business/market evidence. Do not approve because it is fluent. Check wrong-industry meaning, subtle negations, naturalness, scope/brand leakage, language, context, unsupported constraints and compound questions. Similar language is not duplicate unless intent is interchangeable. Compare prior questions for cross-Topic duplicates. checkedUnitKeys must list every unit reviewed. Return issues, not rewritten questions.", { config, business, units: batch, prompts: result.prompts, priorQuestions: prompts.map(p => ({ unitKey: p.unitKey, text: p.text })), sources: evidencePayload(sources.filter(s => batch.some(u => u.sourceIds.includes(s.id)))) }, reviewSchema, data => {
        ...
        if (data.visibilityAssessments.length !== batch.length || new Set(data.visibilityAssessments.map(a => a.unitKey)).size !== batch.length || batch.some(u => !data.visibilityAssessments.some(a => a.unitKey === u.key))) throw new Error("Every question needs an independent brandless-answer assessment");
        data.visibilityAssessments.forEach(validateVisibilityAssessment);
      }, 10000);
      const blockingIssues = review.issues.filter(i => i.kind !== "pool_mismatch");
      if (!blockingIssues.length) {
        for (const unit of batch) {
          const assessment = review.visibilityAssessments.find(a => a.unitKey === unit.key);
          const row = result.prompts.find(p => p.unitKey === unit.key);
          unit.plannedPool = unit.pool;
          unit.pool = routeAssessment(assessment);
          unit.expectedEntityType = assessment.expectedEntityType;
          unit.visibilityAssessment = { ...assessment, policyVersion: MONITORING_POLICY_VERSION, basis: "semantic_review", reviewedPrompt: row.text };
        }
        prompts.push(...result.prompts); semanticReview.checkedUnits += batch.length; break;
      }
      if (attempt) throw new Error(`语义复核未通过：${blockingIssues.map(i => `${i.unitKey}: ${i.reason}`).join("；")}`);
      repairIssues = blockingIssues; semanticReview.repairedBatches++;
    }
  }
  ...
  const artifact = {
    schemaVersion: SCHEMA_VERSION, skillVersion: VERSION, monitoringPolicyVersion: MONITORING_POLICY_VERSION, domain: config.domain, startedAt, generatedAt: new Date().toISOString(),
  ...
  artifact.visibilityReport = visibilitySummary(artifact);
  if (!artifact.visibilityReport.servicePrompts) artifact.researchTrace.warnings.push("暂无通过品牌必要性审查的服务监控问题，保留内容与基准清单，不能直接交付为品牌竞争监控。");
  artifact.optimizationBriefs = artifact.generatedTopics.map(t => ({ topic: t.topic, briefs: t.prompts.map(p => ({ intentUnitId: p.intentUnitId, prompt: p.p, scope: p.scope, pool: p.pool, serviceability: p.serviceabilityStatus, rationale: p.visibilityAssessment.rationale, ...p.visibilityAssessment.optimization, relatedContentUnitKeys: units.find(u => u.intentUnitId === p.intentUnitId)?.relatedContentUnitKeys || [] })) }));
  ...
  artifact.outputCounts = { topics: artifact.generatedTopics.length, prompts: prompts.length, servicePrompts: artifact.visibilityReport.servicePrompts, benchmarkPrompts: artifact.visibilityReport.benchmarkPrompts, citationPrompts: artifact.visibilityReport.citationPrompts, contentPrompts: artifact.visibilityReport.contentPrompts };
  artifact.generationStatus = { mode: "evidence_panel_v3", isFallback: false, status: artifact.deferredUnits.length || business.status !== "confirmed" || !gapReview.saturated || !industryEvidenceAdequate || !artifact.visibilityReport.servicePrompts ? "provisional" : "completed_within_researched_scope", error: "" };
...
export function renderPanel(a) {
  const report = a.coverageReport[0], visibility = a.visibilityReport;
  const pools = [["monitoring_core", "品牌竞争问题"], ["citation_monitoring", "引用观察"], ["content_opportunity", "内容选题"]];
  const scopeName = { brand_core: "客户可承接", industry_benchmark: "行业基准", competitive_whitespace: "竞争机会", out_of_scope_reference: "范围外参考" };
  const sections = a.generatedTopics.map((t, i) => {
    const groups = pools.map(([pool, label]) => {
      const rows = t.prompts.filter(p => p.pool === pool);
      if (!rows.length) return "";
      return `### ${label}（${rows.length}）\n\n| 问题 | 范围 | 为什么这样归类 | 优化动作与证据 |\n|---|---|---|---|\n${rows.map(p => `| ${rowText(p.p)} | ${scopeName[p.scope]} | ${rowText(p.visibilityAssessment.rationale)} | ${rowText(p.visibilityAssessment.optimization.action)}；证据：${rowText(p.visibilityAssessment.optimization.proofNeeded.join("；"))} |`).join("\n")}`;
    }).filter(Boolean).join("\n\n");
    return `## Topic ${i + 1}: ${t.topic}\n\n设计原因：${t.rationale}\n\n${groups}`;
  }).join("\n\n");
...
| 清单 | 问题数 | 用途 |
|---|---:|---|
| 服务优化清单（默认导出） | ${visibility.servicePrompts} | 客户可承接、回答需要具体品牌或产品，并有内容与证据优化路径 |
| 行业竞争基准 | ${visibility.benchmarkPrompts} | 独立观察同品类竞争，包括客户当前不擅长的场景；与服务清单存在重叠，不能相加 |
| 引用观察清单 | ${visibility.citationPrompts} | 观察来源与域名引用，不混同品牌推荐 |
| 内容选题清单 | ${visibility.contentPrompts} | 保留知识、方法和操作需求，用作支持内容，不计入品牌竞争 KPI |

审查标准：如果不出现具体品牌或产品也能完整回答，就不纳入品牌竞争清单；偶然举例、best/top 字眼或一条可选引用不能单独证明监控价值。
${visibility.servicePrompts ? "" : "当前没有足够合格的服务监控问题，不能把内容题补入清单凑数。"}
${visibility.topicsWithoutServiceMonitors.length ? "暂无服务监控问题的主题：" + visibility.topicsWithoutServiceMonitors.join("；") : ""}

## 调研与覆盖范围

读取官网 ${a.crawlReport.effectivePages} 页，执行 ${a.researchTrace.searchLog.length} 条搜索。已研究 ${report.knownUnits} 个意图，生成 ${report.coveredUnits} 条问题，预算暂缓 ${report.deferredUnits} 个意图。
归类为内容选题不代表从行业图谱删除。以上是已研究清单的覆盖情况，不代表全行业真实搜索量或全部互联网需求。
...
准入是语义设计判断，不是实测提及率，也不保证特定品牌会被推荐。默认不塞入自有品牌词。
服务清单与行业基准分别统计，基准包含合格的品牌核心问题；不能通过只保留品牌擅长的问题声称行业领先。
改变问题、用途池或准入规则后，需要重建基线或用共同问题集比较，不能把分母变化当作 GEO 增长。
`;
}

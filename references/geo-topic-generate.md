...
在 auto 模式下，抽取能够完整覆盖高优先级可承接意图的**最小非重叠 Topic 集合**；仅在 manual 模式下恰好输出 `topicCount` 个，并披露因数量限制导致的覆盖缺口。
...
8. **竞品与差异化映射**：使用 `competitorMap` 判断哪些 Topic 需要覆盖直接竞品、替代方案、国家差异、业务线交叉、以及目标品牌的核心差异化角度。
...
11. **覆盖单元建模**：按 [coverage-engine.md](coverage-engine.md) 为 Topic 建立 `cv.cells`，仅包含适用、真实、客户可承接的角色 × JTBD × 意图 × 决策标准组合。
...
- `pc`：覆盖该 Topic 全部 High-priority 和适用 coverage cells 所需的最小 Prompt 数，通常 4-20。
...
ts: 数组
  ...
  - pc: number (4-20) — coverage-derived final prompt count

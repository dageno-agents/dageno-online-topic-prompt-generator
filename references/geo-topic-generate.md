...
在 auto 模式下，抽取能够完整覆盖高优先级可承接意图与竞争决策面的**完整非重叠 Topic 集合**；仅在 manual 模式下恰好输出 `topicCount` 个，并披露因数量限制导致的覆盖缺口。不要把 10 当作自动模式的上限或目标。
...
11. **竞争决策面建模**：先按 [coverage-engine.md](coverage-engine.md) 识别买家选择、拒绝、对比或验证供应商时需要解决的决策面；每个决策面必须有业务证据和可承接能力。
12. **覆盖单元建模**：为 Topic 建立 `cv.cells`，仅包含适用、真实、客户可承接的角色 × JTBD × 决策面 × 意图 × 决策标准组合。不要构造全排列。
